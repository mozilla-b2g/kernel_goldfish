...
goldfish_nfc_irq_bh(unsigned long data)
{
        ...
        static const unsigned long reg[] = {
                REG_RESP,
                REG_RESP,
                REG_NTFN,
                REG_DATA
        ...
        static const unsigned char rcv[] = {
                CTRL_RESP_RCV,
                CTRL_RESP_RCV,
                CTRL_NTFN_RCV,
                CTRL_DATA_RCV
        ...
        for (i = 0; i < ARRAY_SIZE(flags); ++i) {
                ...
                msg = read_nfc_msg(nfc->base+reg[i], GFP_ATOMIC, tags[i]);
                if (IS_ERR(msg))
                        break;
                ...
                iowrite8(rcv[i], nfc->base+REG_CTRL);
                r_wakeup = 1;
        }
...
static int __devexit goldfish_nfc_remove(struct platform_device *pdev)
{
        struct goldfish_nfc *nfc = platform_get_drvdata(pdev);
...
static struct platform_driver goldfish_nfc_driver = {
        ...
        .remove = __devexit_p(goldfish_nfc_remove),
...
static void __exit goldfish_nfc_exit(void)
{
        platform_driver_unregister(&goldfish_nfc_driver);
}

module_init(goldfish_nfc_init);
module_exit(goldfish_nfc_exit);
